...
public class EmpDemo1 {
	public static void main(String[] args) {
		...
		//1 which employee highest salary in department 
		Optional<Employee> collect = emps.stream().collect(Collectors.maxBy(Comparator.comparing(Employee::getSalary)));
		if(collect.isPresent()) {
			System.out.println(collect.get());
			
		//get the employee who is having 2nd highest salary 
			System.out.println("*******1st way**********\n");
			
			
		//1st way -> sort employee in salary  order reverse order then skip first record
			Optional<Employee> sortedemp = emps.stream().sorted(Comparator.comparing(Employee::getSalary).reversed()).
											skip(1).
											findFirst();
			
			if(sortedemp.isPresent()) {
				System.out.println(sortedemp.get());
			}
			//2nd way ->
			System.out.println("********2nd way *******\n");
			List<Employee> collect2 = emps.stream().
							sorted(Comparator.comparing(Employee::getSalary).
											reversed()).
											collect(Collectors.toList());
			System.out.println(collect2.get(1));
			
	List<Employee> collect4	=	emps.stream().sorted(Comparator.comparing(Employee::getSalary).reversed()).collect(Collectors.toList());
			
			 System.out.println(collect4.get(1).salary);
			 Optional<Employee> min= emps.stream().min(Comparator.comparing(Employee::getSalary));
			 System.out.println(min);
			 
			 Double empss =emps.stream().collect(Collectors.averagingDouble(Employee::getSalary));
		
		System.out.println(empss);
		Optional<Employee> emps2=emps.stream().collect(Collectors.maxBy(Comparator.comparing(s->s.getSalary())));
